...
<body>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        #chat-container { position: fixed; bottom: 20px; right: 20px; width: 380px; height: 500px; background: rgba(15,23,42,0.9); backdrop-filter: blur(12px); border: 1px solid rgba(6,182,212,0.2); border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); display: flex; flex-direction: column; z-index: 10000; transition: 0.3s; transform: translateY(20px); opacity: 0; pointer-events: none; font-family: sans-serif; }
        #chat-container.active { transform: translateY(0); opacity: 1; pointer-events: all; }
        #chat-header { padding: 15px; background: rgba(30,41,59,0.5); border-bottom: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; border-radius: 16px 16px 0 0; }
        #chat-messages { flex: 1; padding: 15px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px; background: rgba(2,6,23,0.4); }
        .chat-bubble { max-width: 80%; padding: 10px 14px; border-radius: 12px; font-size: 14px; line-height: 1.4; }
        .chat-user { align-self: flex-end; background: #06b6d4; color: #000; font-weight: 500; }
        .chat-ai { align-self: flex-start; background: #1e293b; color: #fff; border: 1px solid #334155; }
        #chat-input-area { padding: 12px; border-top: 1px solid rgba(255,255,255,0.1); }
        #chat-form { display: flex; gap: 8px; }
        #chat-input { flex: 1; background: #0f172a; border: 1px solid #334155; border-radius: 8px; color: #fff; padding: 8px 12px; outline: none; }
        #chat-send { background: #06b6d4; border: none; padding: 8px 15px; border-radius: 8px; cursor: pointer; color: #000; }
        #chat-toggle-btn { position: fixed; bottom: 25px; right: 25px; width: 60px; height: 60px; background: #06b6d4; border: none; border-radius: 50%; box-shadow: 0 5px 15px rgba(6,182,212,0.4); cursor: pointer; z-index: 9999; display: flex; align-items: center; justify-content: center; transition: 0.3s; }
        #chat-toggle-btn:hover { transform: scale(1.1); }
    </style>

    <button id="chat-toggle-btn" onclick="toggleChat()"><i class="fa-solid fa-comment-dots" style="font-size:24px; color:#000;"></i></button>

    <div id="chat-container">
        <div id="chat-header">
            <span style="font-weight:700; color:#06b6d4;"><i class="fa-solid fa-shield-halved"></i> Sentinel AI</span>
            <button onclick="toggleChat()" style="background:none; border:none; color:#64748b; cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div id="chat-messages">
            <div class="chat-bubble chat-ai">Hello! I am Sentinel AI. How can I help you today?</div>
        </div>
    ...
    <script>
        ...
        async function handleChat(e) {
            ...
            try {
                ...
                container.innerHTML += `<div class="chat-bubble chat-ai">${data.reply}</div>`;
            } catch (err) {
                container.innerHTML += `<div class="chat-bubble chat-ai" style="color:#ef4444">Error connecting to AI.</div>`;
            }
            container.scrollTop = container.scrollHeight;
        }
